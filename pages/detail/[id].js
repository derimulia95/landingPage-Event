...
export default function DetailPage({ detailPage, id }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res = await getData("api/v1/participant/landing-page");
      setData(res.data);
      try {
      } catch (err) {
        console.log(err);
        ...
        <div className="d-flex flex-wrap justify-content-lg-center gap">
          ...
          <div className="d-flex flex-column card-event">
            ...
            <div className="d-flex align-items-center gap-3 mt-3">
              <img
                src={`${process.env.NEXT_PUBLIC_API_IMAGE}/${detailPage?.speaker?.avatar}`}
                alt="semina"
                width="60"
              />
              <div>
...
export async function getServerSideProps(context) {
  const req = await getData(
    `api/v1/participant/landing-page/detail-page/${context.params.id}`
  );
  const res = req.data;
  return {
